...
public class FileController {
    ...
    public ResponseEntity<Object> download(Long write_file_id){

        if(write_file_id == null) return new ResponseEntity<>(null, HttpStatus.BAD_REQUEST);   // 400 에러

        WriteFile file = fileService.findById(write_file_id);
        ...
        try{
            ...
            // 유형이 지정되지 않은 경우
            if(mimeType == null){
                mimeType = "application/octet-stream";      // 일련의 8bit 스트림 타입. 유형이 알려지지 않은 파일에 대해
            }
            ...
            // 원본파일 이름sourcename 으로 다운로드 하게 하기 위한 세팅
