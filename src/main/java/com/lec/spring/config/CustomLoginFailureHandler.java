...
public class CustomLoginFailureHandler implements AuthenticationFailureHandler {
    ...
    private final String DEFAULT_FAILURE_FORWARD_URL = "/user/loginError";  //실패하면 ""로 포워딩
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        ...
        request.setAttribute("errorMessage", errorMessage);                     // login.html 에 th:text="${errorMessage}" 에 보여짐 -> thymeleaf는 model이 있으면 model 에 있는 것을 보여지지만 현재는 필터의 영역이기에 model 사용불가. 그러므로 request. 으로 보여짐
